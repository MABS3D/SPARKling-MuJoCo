--  Dense matrix kernels matching the pinned MuJoCo scalar C algorithms.
...
package MJ.Matrices with SPARK_Mode is
   ...
   function MulMatVec3 (A : Matrix_3; V : MJ.BLAS.Vector_3) return MJ.BLAS.Vector_3 with
     Global => null, Pre => In_Tier0 (A) and then MJ.BLAS.In_Tier0 (V),
     Post => MJ.BLAS.In_Tier1 (MulMatVec3'Result)
       and then MulMatVec3'Result (0) = Dot_3 (A (0, 0), A (0, 1), A (0, 2), V (0), V (1), V (2))
       and then MulMatVec3'Result (1) = Dot_3 (A (1, 0), A (1, 1), A (1, 2), V (0), V (1), V (2))
       and then MulMatVec3'Result (2) = Dot_3 (A (2, 0), A (2, 1), A (2, 2), V (0), V (1), V (2));

   function MulMatTVec3 (A : Matrix_3; V : MJ.BLAS.Vector_3) return MJ.BLAS.Vector_3 with
     Global => null, Pre => In_Tier0 (A) and then MJ.BLAS.In_Tier0 (V),
     Post => MJ.BLAS.In_Tier1 (MulMatTVec3'Result)
       and then MulMatTVec3'Result (0) = Dot_3 (A (0, 0), A (1, 0), A (2, 0), V (0), V (1), V (2))
       and then MulMatTVec3'Result (1) = Dot_3 (A (0, 1), A (1, 1), A (2, 1), V (0), V (1), V (2))
       and then MulMatTVec3'Result (2) = Dot_3 (A (0, 2), A (1, 2), A (2, 2), V (0), V (1), V (2));
...
private
   function MatT_Row (A, B : Matrix; I : Natural) return Real_Array with
     Global => null,
     Pre => Valid (A) and then Valid (B) and then In_Tier0 (A) and then In_Tier0 (B)
       and then A'Length (2) = B'Length (2) and then I in A'Range (1),
     Post => MatT_Row'Result'First = 0 and then MatT_Row'Result'Last = B'Length (1) - 1
       and then In_Tier1 (MatT_Row'Result);
   pragma Postcondition (Static => (for all J in MatT_Row'Result'Range =>
     MatT_Row'Result (J) = MatT_Component (A, B, I, J)));

   --  A whole row is a proof boundary for the four-lane matrix product.
   procedure Fill_MatT_Row (R : in out Matrix; A, B : Matrix; I : Natural) with
     Global => null,
     Pre => Valid (A) and then Valid (B) and then In_Tier0 (A) and then In_Tier0 (B)
       and then A'Length (2) = B'Length (2) and then I in A'Range (1)
       and then R'First (1) = 0 and then R'First (2) = 0
       and then R'Length (1) = A'Length (1) and then R'Length (2) = B'Length (1);
   pragma Postcondition (Static =>
     (for all J in R'Range (2) => R (I, J) in Tier1_Real
       and then R (I, J) = MatT_Component (A, B, I, J))
     and then (for all K in R'Range (1) =>
       (for all L in R'Range (2) =>
         (if K /= I then R (K, L) = R'Old (K, L)))));
end MJ.Matrices;
