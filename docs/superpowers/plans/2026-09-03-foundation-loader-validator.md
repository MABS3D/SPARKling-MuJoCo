...
- **Implementation note (Task 9, proof):** gnatwhy3 runs out of memory on a chain of calls that each update one component of the same many-pointer record (the first two forms of `Allocate_<Group>`, one call or one `new` per array, never finished on `mj-models.adb`: 45 minutes, 3 to 5 GB). The generated code now builds each group with one aggregate of allocating functions (`New_I32`, `New_F64`, `New_U8`, `New_F32`, `New_I64`, postcondition = one layout conjunct), builds `Model` from per-group locals with one aggregate, and frees or reads arrays in chunk procedures of at most 8 (`CHUNK` in `tools/gen.py`; `Free_<Group>_<k>` with cumulative `<Group>_Null_Upto_<k>` postconditions, `Read_<Group>_<k>` with the layout as Pre and Post). Measured: `mj-models.adb` proves in 382 s with a 973 MB peak (998 checks). The same rule applies to `Read_Sizes` (98 writes into `Sizes`) and `Read_Fixed` (about 200 writes into the fixed structs), which are drivers over chunk procedures too. The scaling table is in `docs/toolchain.md`.
- **Implementation note (Task 9, proof):** `Byte_Array` is indexed by `Byte_Index = 0 .. Natural'Last - 1`, not `Natural`: otherwise a buffer spanning the whole index range has a length outside `Integer` and the reader contracts (`Pos <= B'Length`, `Pos'Old + 8 * Size_Count`, `Base + 2 * Bool_Size`) carry unprovable range and overflow checks.
- **Implementation note (Task 9, tooling):** `tools/guarded.ps1` must watch only the descendants of its own command; the first version sampled every gnat process on the machine, so two concurrent guarded runs killed each other and reported each other's peak memory. `alr exec` returns 0 when gnatprove is killed (it prints `exited with code -1`), so the `.spark` files and `--checks-as-errors=on` are the record, never the exit code alone. Corpus-driven clause corrections (reduced CSR inertia with `nC` entries, tendon tree count, saturating Jacobian capacity, tier-1 bounding boxes, mocap weld bodies) are recorded in the spec.
