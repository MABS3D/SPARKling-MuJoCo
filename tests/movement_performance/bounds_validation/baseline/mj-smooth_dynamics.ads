...
package MJ.Smooth_Dynamics with SPARK_Mode is
   ...
   function Transport_Velocity (Linear, Angular, Offset : Vector) return Vector
     with Global => null,
     Pre => Bounded (Linear) and then Bounded (Angular) and then Bounded (Offset),
     Post => Bounded (Transport_Velocity'Result, 1.0e122)
       and then (for all K in Axis => Transport_Velocity'Result (K) =
         Linear (K) + Cross_Local (Angular, Offset) (K));
