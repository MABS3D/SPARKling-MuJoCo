...
def main():
    ...
    paths += [HERE.parent/'movement_bench.adb', REPO/'experimental/smooth/tests/smooth_probe.adb', HERE.parent/'compact_mass/compact_inertia_test.adb', HERE/'bounds_check_test.adb']
    for path in paths:
        target = snapshot/path.relative_to(REPO); target.parent.mkdir(parents=True, exist_ok=True)
        shutil.copy2(path, target)
    builds = {'c': metadata['builds']['c']}
    for variant in ['baseline', 'current', 'checked', 'compact_test', 'bounds_checked', 'bounds_release']:
        ...
        main = {'checked': 'smooth_probe', 'compact_test': 'compact_inertia_test', 'bounds_checked': 'bounds_check_test', 'bounds_release': 'bounds_check_test'}.get(variant, 'movement_bench')
        project = isolated_project(src, work, main)
        text = project.read_text().replace('project Fragment is',
            f'project Fragment is\n   for Main use ("{main}.adb");\n   for Exec_Dir use "bin";')
        if variant in ('baseline', 'current', 'bounds_release'):
        ...
        if variant in ('compact_test', 'bounds_checked', 'bounds_release'):
            result = subprocess.run([str(binary), '1.0'], text=True, capture_output=True, check=True, timeout=120)
    ...
    metadata.update(builds=builds, experiment='equivalent magnitude checks versus bf265c8f interval checks')
