...
package body MJ.Data.Inertia_Phase with SPARK_Mode is
   ...
   procedure Try_Assemble_Compact (D : in out Simulation; Ok : out Boolean)
   is
   ...
   begin
      Ok := False;
      D.Dynamics.Mass (0 .. AR.Count (D.Ancestors) - 1) := [others => 0.0];
      ...
      for I in 0 .. D.Nv - 1 loop
         pragma Loop_Invariant (MJ.Smooth_Dynamics.Work_Array (D.Dynamics.Mass (0 .. AR.Count (D.Ancestors) - 1)));
         if T.Simple_Count (D.Topology, I) > 0 then
            D.Dynamics.Mass (AR.Start (D.Ancestors, I) + AR.Length (D.Ancestors, I) - 1) := T.Fixed_Inertia (D.Topology, I);
         else
            declare
