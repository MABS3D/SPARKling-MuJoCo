   ...
   begin
      ...
      for C in 1 .. Cases loop
         ...
         if Result = Success then
            ...
            declare
               High : Real_Array ((if N = 0 then 1 else Integer'Last - N * N + 1) .. (if N = 0 then 0 else Integer'Last));
               Wrong : Real_Array (7 .. 7);
            begin
               if N > 0 then
                  Get_Mass_Matrix (D, High, Result); Check;
                  if High /= Mass then raise Program_Error with "mass at high array bounds"; end if;
               end if;
               if N /= 1 then
                  Get_Mass_Matrix (D, Wrong, Result); Check (Invalid_Size);
                  if Wrong (7) /= 0.0 then raise Program_Error with "invalid mass output"; end if;
               end if;
               Get_Mass_Matrix (D, Mass, Result); Check;
            end;
         ...
         if Steps > 0 and then not Forces_Current (D) then
            Get_Acceleration (D, Acc, Result); Check (Stale_Results);
            --  A rejected acceleration can leave an assembled mass current;
            --  a successful state update invalidates both independently.
            Get_Mass_Matrix (D, Mass, Result);
            Check (if Mass_Current (D) then Success else Stale_Results);
         end if;
      end loop;
   end;
   Free (D); Free (D);
