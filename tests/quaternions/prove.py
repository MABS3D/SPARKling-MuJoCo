...
def main():
    ...
    command=['gnatprove','-P',str(project),'-u','mj-quaternions.ads','--level=2',
    ...
    report.record_invocation(directory,'mj-quaternions',started,sources,command)
    result=report.main(['--unit','mj-quaternions','--mode','proof','--build-root',str(HERE/'build'),'--since',str(started),'-v'])
    if result:return result
    receipt={'started':started,'command':command,'source_sha256':sources,
             'project_sha256':project_hash,'report_sha256':hashlib.sha256((directory/'mj-quaternions.spark').read_bytes()).hexdigest()}
